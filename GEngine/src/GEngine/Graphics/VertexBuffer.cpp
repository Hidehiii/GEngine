...
namespace GEngine
{
	...
	{
		return Graphics::GetRenderDevice().CreateVertexBuffer(size, sizeInstance, type);
	}

	Ref<VertexBuffer> VertexBuffer::Create(const void* vertices, uint32_t size, uint32_t sizeInstance, VertexTopology type)
	{
		return Graphics::GetRenderDevice().CreateVertexBuffer(vertices, size, sizeInstance, type);
	}

	Ref<IndexBuffer> IndexBuffer::Create(const uint32_t* indices, uint32_t count)
	{
		return Graphics::GetRenderDevice().CreateIndexBuffer(indices, count);
	}
