...
namespace GEngine
{
    ...
    void ModelImporter::ReadVertex(FbxMesh* mesh, int index, Vertex& vertex)
    {
        ...
        vertex.Position.value.x = pCtrlPoint[index].mData[0];
        vertex.Position.value.y = pCtrlPoint[index].mData[1];
        vertex.Position.value.z = pCtrlPoint[index].mData[2];
        vertex.Position.value.w = 1.0f;
    }
